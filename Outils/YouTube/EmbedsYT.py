...
async def embedAlertYT(data,j,nb):
    ...
    embed=discord.Embed(title=data["items"][nb]["snippet"]["title"],url="https://www.youtube.com/watch?v={0}".format(data["items"][nb]["id"]["videoId"]),description="Nouvelle vidéo !",color=0xFF0000)
    ...
    embed.add_field(name="Description",value=data["items"][nb]["snippet"]["description"],inline=True)
    embed.set_image(url=data["items"][nb]["snippet"]["thumbnails"]["medium"]["url"])
    embed.add_field(name="Lien",value="[Lien de la vidéo](https://www.youtube.com/watch?v={0})".format(data["items"][nb]["id"]["videoId"]),inline=True)
    return embed
...
def embedYT(table,page,pagemax,mobile):
    ...
    for i in range(15*(page-1),stop):
        ...
        emote="[{0}](https://youtube.com/channel/{1})".format(table[i]["Nom"],table[i]["Chaine"])
