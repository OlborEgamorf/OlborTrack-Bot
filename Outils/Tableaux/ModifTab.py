...
async def addTableau(ctx,bot,args,curseur):
    ...
    assert len(args)>=4, "Il manque des éléments pour créer le tableau ! Donnez moi dans l'ordre : un emoji, le nombre de réactions nécessaire pour faire apparaître un message et un salon mentionné."
    try:
        if args[1].lower()!="all":
            id=emoteDetector(args[1])[0]
        else:
            id=0
    ...
    try:
        ...
        curseur.execute("INSERT INTO sb VALUES({0},{1},'{2}',{3},{4})".format(num,ctx.message.channel_mentions[0].id,args[1],id,nb))
    except:
        raise AssertionError("Ce couple d'emoji et de salon existe déjà.")

    return createEmbed("Tableau créé","Numéro du tableau : {0}\nEmoji : {1}\nNombre d'utilisations : {2}\nSalon : <#{3}>".format(num,args[1],nb,ctx.message.channel_mentions[0].id),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
...
async def chanTableau(ctx,bot,args,curseur):
    ...
    assert ctx.message.channel_mentions[0].permissions_for(ctx.guild.get_member(bot.user.id)).view_channel==True, "Le salon mentionné n'a pas les permissions nécessaires pour que je puisse le voir."
    assert ctx.message.channel_mentions[0].permissions_for(ctx.guild.get_member(bot.user.id)).send_messages==True, "Le salon mentionné n'a pas les permissions nécessaires pour que je puisse envoyer des messages."
    assert len(args)>=3, "Il manque des éléments pour modifier le salon d'un tableau ! Donnez moi dans l'ordre : le numéro du tableau voulu et le nouveau salon mentionné."
    try:
        star=curseur.execute("SELECT * FROM sb WHERE Nombre={0}".format(args[1])).fetchone()
    except:
    ...
    return createEmbed("Tableau modifié","Numéro du tableau : {0}\nNouveau salon : <#{1}>\nMessages transférés : {2}".format(args[1],ctx.message.channel_mentions[0].id,count),0x220cc9,ctx.invoked_with.lower(),ctx.guild)



async def nbTableau(ctx,args,curseur):
    assert len(args)>=3, "Il manque des éléments pour modifier le nombre d'utilisations nécessaire ! Donnez moi dans l'ordre : le numéro du tableau voulu et le nouveau nombre d'utilisations."
    try:
        star=curseur.execute("SELECT * FROM sb WHERE Nombre={0}".format(args[1])).fetchone()
    except:
        raise AssertionError("Le numéro donné n'est pas valide.")
    assert star!=None, "Le numéro donné ne correspond à aucun tableau actif."
    assert star["Count"]!=args[2], "Le nombre d'utilisations donné est le même que celui actuel."

    curseur.execute("UPDATE sb SET Count={0} WHERE Nombre={1}".format(args[2],star["Nombre"]))
    return createEmbed("Tableau modifié","Numéro du tableau : {0}\nNouveau nombre d'utilisations : {1}".format(args[1],args[2]),0x220cc9,ctx.invoked_with.lower(),ctx.guild)



async def delTableau(ctx,bot,args,curseur,guild):
    assert len(args)>=2, "Il manque le numéro de tableau pour le supprimer !"
    try:
        star=curseur.execute("SELECT * FROM sb WHERE Nombre={0}".format(args[1])).fetchone()
    except:
        raise AssertionError("Le numéro donné n'est pas valide.")
    ...
    curseur.execute("DELETE FROM sb WHERE Nombre={0}".format(args[1]))

    if star["ID"]!=0:
        for i in curseur.execute("SELECT * FROM sbmessages WHERE Nombre={0}".format(args[1])).fetchall():
        ...
        curseur.execute("DELETE FROM sbmessages WHERE Nombre={0}".format(args[1]))
    else:
        curseur.execute("UPDATE sbmessages SET Nombre=0")

    for i in curseur.execute("SELECT * FROM sb WHERE Nombre>{0} ORDER BY Nombre ASC".format(args[1])).fetchall():
    ...
    return createEmbed("Tableau supprimé","Numéro du tableau : {0}".format(args[1]),0x220cc9,ctx.invoked_with.lower(),ctx.guild)