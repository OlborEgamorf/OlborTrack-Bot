...
async def addImage(ctx,bot,option):
    ...
    try:
        embed=createEmbed("Ajout image {0}".format(dictTitres[option]),"Pour ajouter une image {0} à la collection, envoyez l'image que vous voulez.\nElle doit dépasser 256 pixels en largeur et en hauteur. Si les dimensions dépassent 1280x720, elle sera redimensionnée.".format(dictTitres[option]),0xf54269,ctx.invoked_with.lower(),ctx.guild)
