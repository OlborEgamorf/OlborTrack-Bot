...
async def toggleAnniversaire(ctx,bot,chan,guild):
    try:
        ...
        if etat["Statut"]==False or chan:
            embed=createEmbed("Activation anniversaires","Pour activer les anniversaires automatiques, donnez moi le salon dans lequel les messages seront envoyés.",0xf54269,"{0} {1}".format(ctx.invoked_parents[0],ctx.invoked_with.lower()),ctx.guild)
            ...
            embed=createEmbed("Activation anniversaires","Le salon dans lequel les messages seront envoyés sera <#{0}>.\nLe message sera : {1}\n Est-ce bon ? Appuyez sur <:otVALIDER:772766033996021761> pour valider.".format(chan,exemple),0xf54269,"{0} {1}".format(ctx.invoked_parents[0],ctx.invoked_with.lower()),ctx.guild)
            ...
            embed=createEmbed("Désactivation anniversaires","Opération validée !",0xf54269,"{0} {1}".format(ctx.invoked_parents[0],ctx.invoked_with.lower()),ctx.guild)
            await ctx.reply(embed=embed)
        guild.getAnniv()
