...
async def addTwitch(ctx,bot,args,curseur):
    ...
    assert len(args)>=4, "Il manque des éléments pour créer l'alerte ! Donnez moi dans l'ordre : un streamer, un salon mentionné et une description."
    
    if True:
        num=curseur.execute("SELECT COUNT() as Nombre FROM twitch").fetchone()["Nombre"]+1
        stream=createPhrase([args[1]]).lower()[0:-1]
        descip=createPhrase(args[3:len(args)])
    ...
    return createEmbed("Alerte Twitch créée","Numéro de l'alerte : {0}\nStreamer : {1}\nSalon : <#{2}>\nDescription : {3}".format(num,stream,ctx.message.channel_mentions[0].id,descip),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
...
async def chanTwitch(ctx,bot,args,curseur):
    ...
    assert ctx.message.channel_mentions[0].permissions_for(ctx.guild.get_member(bot.user.id)).view_channel==True, "Le salon mentionné n'a pas les permissions nécessaires pour que je puisse le voir."
    assert ctx.message.channel_mentions[0].permissions_for(ctx.guild.get_member(bot.user.id)).send_messages==True, "Le salon mentionné n'a pas les permissions nécessaires pour que je puisse envoyer des messages."
    assert len(args)>=3, "Il manque des éléments pour modifier le salon d'une alerte ! Donnez moi dans l'ordre : le numéro de l'alerte voulue et le nouveau salon mentionné."
    try:
        alert=curseur.execute("SELECT * FROM twitch WHERE Nombre={0}".format(args[1])).fetchone()
    except:
    ...
    return createEmbed("Alerte Twitch modifiée","Numéro de l'alerte : {0}\nNouveau salon : <#{1}>".format(args[1],ctx.message.channel_mentions[0].id),0x220cc9,ctx.invoked_with.lower(),ctx.guild)



async def delTwitch(ctx,bot,args,curseur,guild):
    assert len(args)>=2, "Il manque le numéro de l'alerte pour la supprimer !"
    try:
        alert=curseur.execute("SELECT * FROM twitch WHERE Nombre={0}".format(args[1])).fetchone()
    except:
        raise AssertionError("Le numéro donné n'est pas valide.")
    ...
    curseur.execute("DELETE FROM twitch WHERE Nombre={0}".format(args[1]))

    for i in curseur.execute("SELECT * FROM twitch WHERE Nombre>{0} ORDER BY Nombre ASC".format(args[1])).fetchall():
        curseur.execute("UPDATE twitch SET Nombre={0} WHERE Nombre={1}".format(i["Nombre"]-1,i["Nombre"]))

    return createEmbed("Alerte Twitch supprimée","Numéro de l'alerte : {0}".format(args[1]),0x220cc9,ctx.invoked_with.lower(),ctx.guild)


async def descipTwitch(ctx,bot,args,curseur,guild):
    assert len(args)>=2, "Il manque le numéro de l'alerte pour la modifier !"
    try:
        alert=curseur.execute("SELECT * FROM twitch WHERE Nombre={0}".format(args[1])).fetchone()
    except:
        raise AssertionError("Le numéro donné n'est pas valide.")
    ...
    return createEmbed("Alerte Twitch modifiée","Numéro de l'alerte : {0}\nNouvelle description : {1}".format(args[1],descip),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
