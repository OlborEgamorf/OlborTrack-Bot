...
async def exeUtile(ctx,bot,option):
    ...
    if option=="about":
        embedF=discord.Embed(title="A propos", description="Olbor Track est un bot discord français et fait maison par mes soins sur mon temps libre. Sa principale fonction était de donner des statistiques sur l'activité de votre serveur, et de ses membres.\nDepuis, il s'est étoffé de nombreuses fonctionnalités.\nToutes les données récoltées par le bot ne sortent pas de votre serveur et de la base de donnée du bot. Personne ne peut y accèder à part vous, et sur votre demande.\nC'est pour cela que les statistiques personnelles ne peuvent pas être recherchées.\nLe bot est tout le temps en évolution, et des fonctionnalités vont arriver plus tard.\nIl vaut mieux que le bot possède la permission administrateur, pour ne pas le faire planter en cas de permission manquante. Cela dit, vous pouvez toujours la retirer.\nEn cas de problème, n'hésitez pas à rejoindre le serveur de support.\n**Merci infiniment de l'avoir ajouté.**\nOlborEgamorf.",color=ctx.author.color)
    elif option=="invite":
