...
async def exeUtile(interaction,bot):
    ...
    if interaction.command.name=="about":
        ...
        embed=createEmbed("Olbor Track Companion","Depuis le début de l'année je bosse sur le prototype de Olbor Track Companion, le site internet de OT où l'on pourra consulter les stats, et gérer les outils de vos serveurs. Aujourd'hui la partie stats commence à être aboutie, c'est pour ça que j'envisage des phases de test privés pour avoir des retours sur cet humble travail !\nVoici les différentes phases : \n- Fin de la semaine : accès en Alpha privée pour certaines personnes sélectionnées par moi-même : des amis et des gens qui utilisent beaucoup le bot\n- Fin du mois : accès en Bêta privée pour toutes les personnes ayant le rôle 'Testeur' ou 'Contributeur' sur le serveur de tests + quelques invités\n- Début/mi juin : 2eme Bêta privée sur inscription pour toutes les personnes qui seraient intéressées ! Vous pouvez vous inscrire ici : https://docs.google.com/forms/d/e/1FAIpQLSciRnR5bUkF_eZxGAy4ylb-SPsNl_zMTod9ZMMV6-o4mTj-Ow/viewform?usp=sf_link\n\nVous pourrez retrouver sur ces Bêta :\n- Vraie interface pour consulter les stats d'un serveur\n- Nouveaux graphiques, intéractifs\n- Nouveaux comparateurs, plus clairs et plus modulables\n- Mixes : possibilité de fusionner les stats de plusieurs serveurs\n- Graph Studio + Rapports : centre de commande avec plein de graphiques et d'analyses des stats des serveurs\n- Profil utilisateur avec vos badges, titres et stats personnelles\n- Et + !",0xfcfc03,interaction.command.name,bot.user)
