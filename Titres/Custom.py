...
async def achatCustom(ctx,bot):
    try:
        ...
        connexionUser.close()
        assert coins>=5000, "Vous n'avez pas assez d'OT Coins pour acheter le surnom personnalisé !"

        embed=createEmbed("Surnom personnalisé","Vous êtes sur le point de personnaliser votre surnom, pour *5000 <:otCOINS:873226814527520809>*.\nVous possèdez {0} <:otCOINS:873226814527520809> au total et en aurez {1} <:otCOINS:873226814527520809> après la transaction.\nAppuyez sur <:otVALIDER:772766033996021761> pour confirmer l'achat, et passer à la configuration.".format(coins,coins-5000),0xf58d1d,ctx.invoked_with.lower(),ctx.author)
        ...
        embed=createEmbed("Surnom personnalisé","Donnez moi le surnom que vous voulez avoir, et qui sera mis derrière votre titre. Il ne doit pas dépasser 10 caractères.",0xf58d1d,ctx.invoked_with.lower(),ctx.author)

        message=await ctx.reply(embed=embed)

        def checkCustom(mess):
            try:
                assert len(mess.content)<=10
            except:
                return False
        ...
        connexion.close()
        if titre==None:
            embed=createEmbed("Surnom personnalisé","Votre nom affiché dans les classements mondiaux sera désormais **{0}, Inconnu**.\nValidez ce choix avec <:otVALIDER:772766033996021761>.\nVous ne pourrez plus revenir en arrière, chaque changement de surnom coûte 5000 <:otCOINS:873226814527520809>.".format(custom),0xf58d1d,ctx.invoked_with.lower(),ctx.author)
        else:
            embed=createEmbed("Surnom personnalisé","Votre nom affiché dans les classements mondiaux sera désormais **{0}, {1}**.\nValidez ce choix avec <:otVALIDER:772766033996021761>.\nVous ne pourrez plus revenir en arrière, chaque changement de surnom coûte 5000 <:otCOINS:873226814527520809>.".format(custom,titre["Nom"]),0xf58d1d,ctx.invoked_with.lower(),ctx.author)
        ...
        coins=curseurUser.execute("SELECT * FROM coins").fetchone()["Coins"]
        assert coins>=5000, "Vous n'avez pas assez d'OT Coins pour acheter le surnom personnalisé !"
        curseurUser.execute("UPDATE coins SET Coins=Coins-5000")
        ...
        embed=createEmbed("Surnom personnalisé","Transaction effectuée avec succès.\nVotre surnom est maintenant : **{0}**".format(custom),0xf58d1d,ctx.invoked_with.lower(),ctx.author)
