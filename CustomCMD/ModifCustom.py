...
async def addCCMD(ctx,args,nom,command,curseur):
    ...
    quote=createPhrase(args[2:len(args)])
    ...
    return createEmbed("Commande ajoutée","Nom : {0}\nContenu : {1}".format(nom,quote),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
...
async def editCCMD(ctx,args,nom,curseur):
    ...
    return createEmbed("Commande modifiée","Nom : {0}\nNouveau contenu : {1}".format(nom,quote),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
...
async def helpCCMD(ctx,args,nom,curseur):
    ...
    return createEmbed("Contenu d'aide modifié","Commande : {0}\nDescription : {1}".format(nom,quote),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
...
async def embedCCMD(ctx,nom,command,curseur):
    ...
    return createEmbed("Embed {0}activé".format(dictBool[bool(command["Embed"])]),"Commande : {0}".format(nom),0x220cc9,ctx.invoked_with.lower(),ctx.guild)

async def authorCCMD(ctx,args,nom,curseur):
    """Change l'auteur pour un embed custom"""
    if args[2].lower()=="user" or args[2].lower()=="guild":
        author=args[2].lower()
    elif len(ctx.message.mentions)>0:
        author=ctx.message.mentions[0].id
    elif args[2].lower()=="del":
        author="None"
    else:
        author=createPhrase(args[2:len(args)])
    curseur.execute("UPDATE custom SET Author='{0}' WHERE Nom='{1}'".format(author,nom))
    return createEmbed("Auteur d'embed modifié","Commande : {0}\nAuteur : {1}".format(nom,author),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
...
async def imageCCMD(ctx,args,nom,curseur,option):
    ...
    return createEmbed("{0} d'embed modifiée".format(option),"Commande : {0}\n{1} : {2}".format(nom,option,image),0x220cc9,ctx.invoked_with.lower(),ctx.guild)
...
async def colorCCMD(ctx,args,nom,curseur):
    ...
    assert args[2].lower() in listColor, "Cette couleur n'est pas disponible. Liste des couleurs : blue, yellow, orange, green, pink, black, white, violet, cyan, red, turquoise, dblue, dgreen, user ou olbor"
    curseur.execute("UPDATE custom SET Color='{0}' WHERE Nom='{1}'".format(args[2].lower(),nom))
    return createEmbed("Couleur d'embed modifiée","Commande : {0}\nCouleur : {1}".format(nom,args[2].lower()),0x220cc9,ctx.invoked_with.lower(),ctx.guild)