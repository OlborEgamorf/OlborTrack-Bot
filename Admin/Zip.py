...
async def exeZip(ctx,bot):
    ...
    ziph = zipfile.ZipFile("SQL/{0}/ZIP.zip".format(ctx.guild.id), 'w', zipfile.ZIP_DEFLATED)
    ...
    await ctx.author.send(embed=embedF,file=discord.File("SQL/{0}/ZIP.zip".format(ctx.guild.id)))
    return