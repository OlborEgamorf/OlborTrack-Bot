...
async def svPersoModoSlash(interaction,bot,option):
    ...
    curseur.execute("INSERT INTO commandes VALUES({0},{1},'savezvous','{2}','None','None','None','None',1,{3},'countDesc',False)".format(interaction.id,interaction.user.id,option,pagemax))
    ...
    if pagemax>1:
        await sendSlash(interaction,embed,curseur,connexion,1,pagemax,customView=ViewControls(mobile=False,tri=False,graph=False))
    else:
        await sendSlash(interaction,embed,curseur,connexion,1,pagemax,customView=ViewControls(mobile=False,tri=False,graph=False,droite=False,gauche=False,page=False))
