...
@OTCross
async def startGameP4Cross(ctx,bot,game,inGame,gamesP4):
    ...
    while game.playing:
        ...
        if add[0]==True:
            if game.tab.checkTab(add[1],add[2],turn+1)==True:
                ...
                wins=exeJeuxSQL(game.joueurs[turn].id,game.joueurs[lose].id,"W","OT",curseurOT,2,"P4",game.tours)
                ...
                for i in game.messages:
                    ...
                    await sendCarte(bot.get_user(game.joueurs[turn].id),"P4",wins,"cross",i.channel)
                    await unpin(i)
                game.playing=False
            else:
