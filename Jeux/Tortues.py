...
choix={"ot:tortues1":0,"ot:tortues2":1,"ot:tortues3":2,"ot:tortues4":3,"ot:tortues5":4,"ot:tortuesbleu":"bleue","ot:tortuesjaune":"jaune","ot:tortuesrouge":"rouge","ot:tortuesvert":"verte","ot:tortuesviolet":"violette"}
...
class JoueurTortue(JoueurBase):
    def __init__(self,user,interaction,tortue):
...
class JeuTortues(JeuBase):
    ...
    def addPlayer(self,user,interaction):
        ...
        self.joueurs.append(JoueurTortue(user,interaction,tortue))
    ...
    async def sendTortues(self):
        for i in self.joueurs:
            await i.interaction.followup.send(embed=createEmbed("Course des tortues : {0}".format(i.couleur),"Votre couleur est : {0} {1}".format(i.couleur,dictEmote[i.couleur]),dictColor[i.couleur],"tortues",i.user),ephemeral=True)
        self.giveCards()
    ...
    async def play(self,bot,interactionUser):
        ...
        def check(interaction):
            return interaction.data["custom_id"] in ("ot:tortues1","ot:tortues2","ot:tortues3","ot:tortues4","ot:tortues5") and interaction.message.id==interactionUser.message.id and self.joueurs[self.turn].id==interaction.user.id

        try:
            interaction=await bot.wait_for('interaction', check=check, timeout=40)
            ...
            interID=interaction.data["custom_id"]
            if self.joueurs[self.turn].jeu[choix[interID]].couleur=="multi":
                carte=self.joueurs[self.turn].jeu[choix[interID]]
                valeur=carte.valeur
                eff=await interaction.followup.send("<@{0}>, choisissez une tortue avec les boutons <:OTTbleu:860119157491892255> <:OTTjaune:860119157688631316> <:OTTrouge:860119157495693343> <:OTTvert:860119157331853333> <:OTTviolet:860119157672247326>.".format(self.joueurs[self.turn].id),ephemeral=True)

                def check(interaction):
                    return interaction.data["custom_id"] in ("ot:tortuesbleu","ot:tortuesjaune","ot:tortuesrouge","ot:tortuesvert","ot:tortuesviolet") and interaction.message.id==interactionUser.message.id and self.joueurs[self.turn].id==interaction.user.id
                try:
