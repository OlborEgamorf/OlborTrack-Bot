...
def graphPerso(ligne,ctx,option,bot,period,categ):
    ...
    if ligne["Commande"]=="first":
        ...
        if period=="mois":
            ...
            table=curseur.execute("SELECT * FROM firstA WHERE Annee<>'GL' ORDER BY DateID ASC").fetchall()
        for i in table:
            i["Rank"]=0
    else:
        if period=="mois":
    ...
    for i in table:
        dictX={"Compteur":"{0}e".format(i["Rank"]),"Rang":formatCount(option,i["Count"]),"First":""}
        dictY={"Compteur":i["Count"],"Rang":i["Rank"],"First":i["Count"]}
    ...
    if ligne["Commande"]=="first":
        ...
        plt.title("Premiers - Périodes",fontsize=12)
    else:
        if not obj:
