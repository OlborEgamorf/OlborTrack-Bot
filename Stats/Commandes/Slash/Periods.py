...
from Core.Fonctions.GetTable import getTablePeriods
...
async def statsPeriods(interID:int,interaction:discord.Interaction,guildOT:OTGuild,bot,curseur:CustomCursor,ligne:dict):
    if True:
        option = ligne["Option"]
        ...
        if obj == None:
            column = dictColumn[option]
        else:
            column = "User"
        
        table=getTablePeriods(curseur,option,author,"M",column,ligne["Tri"])
        ...
        if pagemax == 1:
            ...
            table=getTablePeriods(curseur,option,author,"A",column,ligne["Tri"])
        else:
            table = table[15*(page-1):15*(page)]
            
        if option in ("Voice","Messages"):
            iconAuth = "user"
            obj=None
        else:
            iconAuth = "guild"
        ...
        return await statsEmbed(interaction,guildOT,bot,table,ligne,iconAuth,"Périodes {0}".format(option.lower()),option,"Mois",obj=obj)
