...
def pagemaxHomeJour(curseur,jour,mois,annee,period):
    listeOptions=[]
    if period=="jour":
        table=curseur.execute("SELECT DISTINCT Type FROM ranks WHERE Jour='{0}' AND Mois='{1}' AND Annee='{2}' ORDER BY Type ASC".format(jour,mois,annee)).fetchall()
    elif period=="mois":
