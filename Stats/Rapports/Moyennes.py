...
def descipMoyennes(option,result):
    ...
    dictBloc={
        ...
        "Voicechan":"Salons utilisés : {0}\nMoyenne temps en vocal par salon : {1}\nMeilleur salon : {2}\nMédiane : {3}\nPire salon : {4}".format(numb,tempsVoice(round(count/numb,3)),tempsVoice(result[0]["Count"]),tempsVoice(result[len(result)//2]["Count"]),tempsVoice(result[len(result)-1]["Count"])),
    }
