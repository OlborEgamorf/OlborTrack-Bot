    ...
    if result!=[]:
        ...
        for i in range(start,stop):
            ...
            for j in range(6 if len(obj)>6 else len(obj)):
                if period=="jour":
                    descip+="**{0} {1} 20{2}** : {3} - {4}e\n".format(obj[j]["Jour"],tableauMois[obj[j]["Mois"]],obj[j]["Annee"],obj[j]["Count"],obj[j]["Rank"])
                elif obj[j]["Annee"]=="GL":
                    descip+="**Global** : {0} - {1}e\n".format(obj[j]["Count"],obj[j]["Rank"])
                else:
                    descip+="**{0} 20{1}** : {2} - {3}e\n".format(tableauMois[obj[j]["Mois"]],obj[j]["Annee"],obj[j]["Count"],obj[j]["Rank"])
