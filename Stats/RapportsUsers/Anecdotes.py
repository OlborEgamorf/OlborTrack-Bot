...
def anecdotesSpe(date,guildOT,bot,guild,option,page,pagemax,period,user):
    if period=="jour":
    ...
    dictPremier={"Salons":"Le salon le plus actif est {0}{1}{2}","Freq":"L'heure la plus active est {0}{1}{2}","Emotes":"L'emote la plus utilisée est {0}{1}{2}","Reactions":"La réaction la plus utilisée est {0}{1}{2}","Messages":"Le membre le plus actif est {0}{1}{2}"}
        ...
        if period=="jour":
            for i in curseur.execute("SELECT * FROM ranks WHERE DateID={0} AND Type='{1}'".format(date[2]+date[1]+date[0],option)).fetchall():
                ...
                if best["DateID"]==int(date[2]+date[1]+date[0]):
                    records.append({"ID":i["ID"],"Count":best["Count"]})
        elif period=="mois":
            for i in result:
                ...
                if tablePerso["Mois"]==tableauMois[date[0]] and tablePerso["Annee"]==date[1]:
                    records.append({"ID":i["ID"],"Count":tablePerso["Count"]})
        elif period=="annee":
            for i in result:
    ...
    if option in ("Freq","Messages","Salons"):
        dictSilent={"Messages":"Sur les **{0}** humains de ce serveur, **{1}** ont été actifs à cette date, soit {2}%.","Salons":"Sur les **{0}** salons existants de ce serveur, **{1}** ont été actifs à cette date, soit {2}%.","Freq":"Sur les 24 heures qui composent une journée, **{1}** ont vu des messages, soit {2}%."}
        count=0
        if option=="Messages":
    ...
    if period=="jour":
        dictDivers={"Messages":9,"Salons":9,"Freq":9,"Emotes":8,"Reactions":6}
        somme=curseur.execute("SELECT Count FROM ranks WHERE DateID={0} AND Type='Divers' AND ID={1}".format(date[2]+date[1]+date[0],dictDivers[option])).fetchone()["Count"]
        ...
        if record["Count"]==somme:
            dictRecord={"Messages":"Il n'y a jamais eu autant de messages envoyés qu'aujourd'hui, avec un total de **{0}**","Freq":"Il n'y a jamais eu autant de messages envoyés qu'aujourd'hui, avec un total de **{0}**","Salons":"Il n'y a jamais eu autant de messages envoyés qu'aujourd'hui, avec un total de **{0}**","Emotes":"Il n'y a jamais eu autant d'emotes utilisées qu'aujourd'hui, avec un total de **{0}**","Reactions":"Il n'y a jamais eu autant de réactions utilisées qu'aujourd'hui, avec un total de **{0}**"}
        else:
            dictRecord={"Messages":"Aujourd'hui **{0}** messages ont été envoyés. La date record est le **{1}/{2}/{3}** avec **{4}** messages.","Freq":"Aujourd'hui **{0}** messages ont été envoyés. La date record est le **{1}/{2}/{3}** avec **{4}** messages.","Salons":"Aujourd'hui **{0}** messages ont été envoyés. La date record est le **{1}/{2}/{3}** avec **{4}** messages.","Emotes":"Aujourd'hui **{0}** emotes ont été utilisées. La date record est le **{1}/{2}/{3}** avec **{4}** emotes.","Reactions":"Aujourd'hui **{0}** réactions ont été utilisées. La date record est le **{1}/{2}/{3}** avec **{4}** réactions."}
        embed.add_field(name="Total",value=dictRecord[option].format(somme,record["Jour"],record["Mois"],record["Annee"],record["Count"]),inline=False)
    ...
    if hier!=None and period!="global":
        ...
        for i in result:
            ...
            if count==None:
                ...
                plus+=1
        dictPlusMoins={"Messages":"Parmi les membres actifs à cette date, **{0}** l'ont été **moins** que la période précédente, **{1} plus** et **{2} ne l'avaient pas été**.","Salons":"Parmi les salons actifs à cette date, {0} l'ont été moins que la période précédente, {1} plus et {2} ne l'avaient pas été.","Freq":"Parmi les heures actives à cette date, {0} l'ont été moins que la période précédente, {1} plus et {2} ne l'avaient pas été.","Emotes":"Parmi les emotes utilisées à cette date, {0} l'ont été moins que la période précédente, {1} plus et {2} ne l'avaient pas été.","Reactions":"Parmi les réactions utilisées à cette date, {0} l'ont été moins que la période précédente, {1} plus et {2} ne l'avaient pas été."}
        embed.add_field(name=title,value=dictPlusMoins[option].format(moins,plus,new),inline=False)

    return embedRapport(guild,embed,date,"Section {0} : anecdotes".format(dictSection[option]),page,pagemax,period,user) 