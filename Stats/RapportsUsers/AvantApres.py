import discord
from Stats.RapportsUsers.OlderEarlier import getEarlierAnnee, getEarlierJour, getEarlierMois, getOlderJour, hierMAG
...
def avantapresSpe(date,guildOT,bot,guild,option,page,pagemax,period,user):
    ...
    if period=="jour":
        ...
        demain=getEarlierJour(date[0],date[1],date[2],curseur,option,user)
    ...
    if hier!=None:
        if period=="jour":
            resultH=curseur.execute("SELECT *,IDComp AS ID FROM objs WHERE Jour='{0}' AND Mois='{1}' AND Annee='{2}' AND Type='{3}' AND ID={4} ORDER BY Count DESC".format(hier[0],hier[1],hier[2],option,user)).fetchall()
    ...
    if period=="jour":
        embed.add_field(name="{0}/{1}/{2}".format(date[0],date[1],date[2]),value=descipGlobal(option,result,0,stop,guildOT,bot,hier,period,user),inline=True)
    elif period=="mois":
    ...
    if demain!=None:
        if period=="jour":
            resultD=curseur.execute("SELECT *,IDComp AS ID FROM objs WHERE Jour='{0}' AND Mois='{1}' AND Annee='{2}' AND Type='{3}' AND ID={4} ORDER BY Count DESC".format(demain[0],demain[1],demain[2],option,user)).fetchall()
