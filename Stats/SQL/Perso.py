def persoSQL(base,id,idComp,rank,count,date):
    ...
    etat=base.execute("SELECT Count,Rank,ID FROM {0} WHERE ID={1}".format("perso"+date[0]+date[1]+str(idComp),id)).fetchone()
    if etat==None:
        try:
        ...
        base.execute("UPDATE {0} SET Count={1}+Count, Rank={2} WHERE ID={3}".format("perso"+date[0]+date[1]+str(idComp),count,rank,id))  